...
var (
	...
	ErrAPIKeyModelRequired     = infraerrors.BadRequest("API_KEY_MODEL_REQUIRED", "a model must be selected for this group")
	ErrAPIKeyModelNotAvailable = infraerrors.BadRequest("API_KEY_MODEL_NOT_AVAILABLE", "selected model is not available in this group")
	ErrAPIKeyModelNotAllowed   = infraerrors.Forbidden("API_KEY_MODEL_NOT_ALLOWED", "this API key is restricted to another model")
...
func validateAPIKeyAllowedModel(group *Group, model string) error {
	model = strings.TrimSpace(model)
	if model == "" {
		if group != nil && group.ModelsListConfig.Enabled {
			return ErrAPIKeyModelRequired
		}
		return nil
	}
	if group == nil || !group.ModelsListConfig.Enabled {
		return ErrAPIKeyModelNotAvailable
	}
...
func (s *APIKeyService) Create(ctx context.Context, userID int64, req CreateAPIKeyRequest) (*APIKey, error) {
	...
	if req.GroupID != nil {
		selectedGroup, err = s.groupRepo.GetByID(ctx, *req.GroupID)
		if err != nil {
			return nil, fmt.Errorf("get group: %w", err)
		}

		// 检查用户是否可以绑定该分组
		if !s.canUserBindGroup(ctx, user, selectedGroup) {
			return nil, ErrGroupNotAllowed
		}
	}
	allowedModel := strings.TrimSpace(req.AllowedModel)
	if err := validateAPIKeyAllowedModel(selectedGroup, allowedModel); err != nil {
		return nil, err
	}
...
func (s *APIKeyService) Update(ctx context.Context, id int64, userID int64, req UpdateAPIKeyRequest) (*APIKey, error) {
	...
	if req.AllowedModel != nil {
		allowedModel := strings.TrimSpace(*req.AllowedModel)
		if err := validateAPIKeyAllowedModel(selectedGroup, allowedModel); err != nil {
			return nil, err
		}
		apiKey.AllowedModel = allowedModel
	} else if groupChanged {
		if err := validateAPIKeyAllowedModel(selectedGroup, ""); err != nil {
			return nil, err
		}
		apiKey.AllowedModel = ""
	}
