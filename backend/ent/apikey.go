...
type APIKey struct {
	...
	// Exact model this API key may call; empty means unrestricted
...
func (*APIKey) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case apikey.FieldKey, apikey.FieldName, apikey.FieldAllowedModel, apikey.FieldStatus:
...
func (_m *APIKey) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case apikey.FieldAllowedModel:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field allowed_model", values[i])
			} else if value.Valid {
				_m.AllowedModel = value.String
			}
...
func (_m *APIKey) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("allowed_model=")
	builder.WriteString(_m.AllowedModel)
	builder.WriteString(", ")
